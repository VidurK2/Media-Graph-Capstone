...
# Load your DataFrame
df = pd.read_csv('/home/vidur/mediagraph/Python/output_final2.csv')
...
entities_df.to_csv('/home/vidur/mediagraph/data/entities2020.csv', index=False)
...
for index, row in df.iterrows():
    ...
    for (entity1, type1), (entity2, type2) in combinations(entities, 2):
        key = tuple(sorted([(entity1, type1), (entity2, type2)]))
        month_year = datetime.strptime(date, '%Y-%m-%d').strftime('%m-%Y')  # Convert date to 'MM-YYYY' format
...
# Create DataFrame for relationships
relationships_df = pd.DataFrame(relationships_list)
relationships_df.to_csv('/home/vidur/mediagraph/data/cooccurrences_with_dates2020.csv', index=False)
